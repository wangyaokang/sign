...
public class ClassController extends AbstractController {
    ...
    public Output modify(Input input) {
        Output result = new Output();
        TbClass TbClass = tbClassService.get(input.getLong("id"));
        if (null == TbClass) {
    ...
    public Output queryClasses(Input input) {
        Output result = new Output();
        List<TbClass> TbClassList = tbClassService.query();
        ...
        try {
            ...
            if (tbStudentList.size() != 0) {
                ...
                for (int i = 0; i < len; i++) {
                    ...
                    if(i % 10 == 0 && i >= 10) {
                        tbStudentService.updateBatch(paramList);
                        paramList.clear();
                    } else if (i == len - 1) {
                        tbStudentService.updateBatch(paramList);
                    }
    ...
    public Output getClassesListByAdmin(Input input) {
        ...
        List<TbClass> TbClassList = tbClassService.query(params);
        if (TbClassList.size() == 0) {
    ...
    public Output getClassesListFormElective(Input input) {
        ...
        for (TbElective tbElective : tbElectiveList) {
            TbClass TbClass = tbClassService.get(tbElective.getTbClass().getId());
    ...
    public Map<String, Object> toMap(TbClass TbClass) {
        ...
        List<TbAdmin> teacherList = tbAdminService.query(param);
